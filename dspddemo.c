...
void insertFlightPlanAndUpdateFile(AVLBucket **bucketList, int flightId, Time departureTime, Time ETA) {
    insertFlightPlan(bucketList, flightId, departureTime, ETA); // Insert the flight plan

    // Open the file in append mode to add the new flight plan
    ...
    // Write the new flight plan to the file
    fprintf(fptr, "%d %d %d %d %d\n", flightId, departureTime.hrs, departureTime.min, ETA.hrs, ETA.min);

    fclose(fptr); // Close the file
}

void deleteFlightPlanAndUpdateFile(AVLBucket **bucketList, int flightId, Time departureTime) {
    AVLBucket *updatedBucketList = deleteFlightPlan(*bucketList, flightId, departureTime); // Delete the flight plan

    // Open the file in read mode to read the existing flight plans
    ...
    // Open a temporary file in write mode to write the flight plans excluding the deleted one
    ...
    // Read flight plans from the original file, exclude the deleted one, and write to the temporary file
    int tempFlightId, tempDepartHrs, tempDepartMin, tempETAHrs, tempETAMin;
    while (fscanf(fptr, "%d %d %d %d %d", &tempFlightId, &tempDepartHrs, &tempDepartMin, &tempETAHrs, &tempETAMin) != EOF) {
        if (tempFlightId != flightId || (tempFlightId == flightId && (tempDepartHrs != departureTime.hrs || tempDepartMin != departureTime.min))) {
            fprintf(tempFile, "%d %d %d %d %d\n", tempFlightId, tempDepartHrs, tempDepartMin, tempETAHrs, tempETAMin);
        }
    }
    ...
    // Update the bucket list pointer
...
int main()
{
    ...
    {
        while (fscanf(fptr, "%d %d %d %d %d", &flightId, &departTime.hrs, &departTime.min, &ETA.hrs, &ETA.min) != EOF)
        {
    ...
    while (1)
    {
        ...
        case 1:
            ...
            insertFlightPlanAndUpdateFile(&bucketList, flightId, departTime, ETA);
            Print(bucketList);
            break;
        case 2:
            ...
            printf("Enter Departure Time (hrs min)\n");
            scanf("%d %d", &departTime.hrs, &departTime.min);
            deleteFlightPlanAndUpdateFile(&bucketList, flightId, departTime);
            Print(bucketList);
            break;
        case 3:
